...
public class CartServiceImpl implements CartService {
    ...
    public Mono<CartResponse> addToCart(UUID accountId, AddToCartRequest request) {
        return variantRepository.findById(request.getVariantId())
                .flatMap(variant -> {
                    ...
                    return orderRepository.findAllByAccountId(accountId) // Lấy đơn hàng hiện tại của user
                            .filter(order -> OrderStatus.PENDING.name().equals(order.getOrderStatus())) // Chỉ lấy đơn
                                                                                                        // hàng có trạng
                                                                                                        // thái PENDING
                                                                                                        // (đang chờ
                                                                                                        // thanh toán)
                            .next()// Nếu có đơn hàng PENDING, sử dụng đơn đó, nếu không có thì tạo mới một đơn
                                   // hàng mới
    ...
    public Mono<CartResponse> updateItemQuantity(UUID accountId, UUID orderItemId, int quantity) {
        return orderRepository.findAllByAccountId(accountId)
                ...
                .flatMap(order -> orderItemRepository.findById(orderItemId)
                        .flatMap(item -> {
                            if (!item.getOrderId().equals(order.getId())) {
                                return Mono.error(
                                        new IllegalArgumentException("Order item không thuộc về đơn hàng của user"));
                            }
                            if (quantity < 1) {
                                return Mono.error(new IllegalArgumentException("Số lượng phải lớn hơn 0"));
                            }
                            return variantRepository.findById(item.getVariantId())
                                    .flatMap(variant -> {
                                        if (variant.getStockQuantity() < quantity) {
                                            return Mono.error(new IllegalStateException("Không đủ hàng trong kho"));
                                        }
                ...
                .flatMap(this::getMyCart); // Sau khi cập nhật số lượng, trả về thông tin giỏ hàng mới nhất để UI có thể
                                           // cập nhật lại
    ...
    public Mono<CartResponse> removeItemFromCart(UUID accountId, UUID orderItemId) {
        return orderRepository.findAllByAccountId(accountId)
                ...
                .flatMap(this::getMyCart); // Sau khi xóa item, trả về thông tin giỏ hàng mới nhất để UI có thể cập nhật
                                           // lại
