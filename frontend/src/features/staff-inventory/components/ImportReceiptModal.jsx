...
export default function ImportReceiptModal({ receipt, onClose }) {
  ...
  return (
    <>
      <button type="button" aria-label="Đóng modal phiễu nhập kho" onClick={onClose} className="fixed inset-0 bg-black/50 z-50 cursor-pointer border-none" />
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4 text-gray-800 text-left">
        <div className="bg-white rounded shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
              ...
              <table className="w-full text-sm">
                ...
                <tbody className="divide-y divide-gray-50">
                  {rows.map((r, i) => {
                    ...
                    return (
                      <tr key={r?.id ?? r?.code ?? r?.name ?? r?.key ?? r?.val ?? r}>
          ...
          <div className="flex gap-3 px-6 pb-6">
            <button aria-label="Đóng phiếu nhập kho" type="button" onClick={onClose} className="flex-1 py-2.5 border border-gray-200 rounded text-sm font-medium text-gray-600 hover:bg-gray-50 cursor-pointer bg-white">
              Đóng
            </button>
            <button aria-label="In phiếu nhập kho" type="button" onClick={() => window.print()} className="flex-1 py-2.5 bg-teal-600 hover:bg-teal-700 text-white rounded text-sm font-semibold cursor-pointer transition-colors flex items-center justify-center gap-2 border-none">
