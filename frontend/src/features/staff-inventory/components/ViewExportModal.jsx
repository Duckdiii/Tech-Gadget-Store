...
export default function ViewExportModal({ log, onClose }) {
  ...
  return (
    <>
      <button type="button" aria-label="Đóng xem phiếu xuất kho" onClick={onClose} className="fixed inset-0 bg-black/50 z-50 cursor-pointer border-none" />
      <div className="fixed inset-0 flex items-center justify-center z-50 p-4">
        <div className="bg-white rounded shadow-2xl w-full max-w-lg max-h-[90vh] overflow-y-auto">
          ...
          <div className="flex gap-3 px-6 pb-6">
            <button aria-label="Đóng chi tiết phiếu xuất" type="button" onClick={onClose} className="flex-1 py-2.5 border border-gray-200 rounded text-sm font-medium text-gray-600 hover:bg-gray-50 cursor-pointer bg-white">Đóng</button>
          </div>
        </div>
      </div>
    </>
