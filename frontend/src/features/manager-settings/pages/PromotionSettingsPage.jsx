...
export default function PromotionSettingsPage() {
  ...
  // pagination/search for this endpoint).
        ...
        <div className="grid grid-cols-[1fr_290px] gap-5">
          ...
          <div className="bg-white rounded border border-gray-200 overflow-hidden">
            ...
            {loading ? (
              ...
              <div className="px-5 py-8 text-sm text-gray-400 text-center">Không tìm thấy chiến dịch phù hợp</div>
            ) : (
          ...
          {/* RIGHT column: Active promo codes từ DB */}
          <div className="flex flex-col gap-4">
            <div className="bg-white rounded border border-gray-200 p-4">
              <h2 className="text-sm font-bold text-gray-800 mb-3">Mã Khuyến Mãi Đang Hoạt Động</h2>

              {loading ? (
                <p className="text-xs text-gray-400 text-center py-4">Đang tải...</p>
              ) : promotions.filter(p => p.active).length === 0 ? (
                <p className="text-xs text-gray-400 text-center py-4">Không có mã đang hoạt động</p>
              ) : (
                <div className="space-y-3">
                  {promotions
                    ...
                    .slice(0, 5)
                    .map(p => {
                      const expiry = p.endAt
                        ? new Date(p.endAt).toLocaleDateString('vi-VN')
                        : 'Không giới hạn'
                      return (
                        <div key={p.id} className="border border-gray-200 rounded p-3">
                          <div className="flex items-center justify-between mb-2">
                            <span className="font-bold text-sm px-2.5 py-1 rounded bg-orange-50 text-[#C4350A] tracking-wider">
                              {p.code}
                            </span>
              ...
              {promotions.filter(p => p.active).length > 5 && (
                <p className="text-xs text-gray-400 text-center mt-3">
                  +{promotions.filter(p => p.active).length - 5} mã khác đang hoạt động
                </p>
              )}
            </div>
          </div>
