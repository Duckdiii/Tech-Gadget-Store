  ...
  return (
    <div className={`w-full h-full bg-gradient-to-br ${bg} flex items-center justify-center`}>
      <svg className="w-10 h-10 text-white/70" fill="none" stroke="currentColor" strokeWidth={1.5} viewBox="0 0 24 24">
        {PRODUCT_ICONS[category] || <path strokeLinecap="round" strokeLinejoin="round" d="M20 7l-8-4-8 4m16 0l-8 4m8-4v10l-8 4m0-10L4 7m8 10V11" />}
      </svg>
    </div>
  )
}
...
function StockBadge({ stock }) {
  const [dot, text, label] = STOCK_MAP[stock] || STOCK_MAP.in_stock
  return (
          ...
          <div className="flex items-center gap-2 shrink-0">
            ...
            <div className="flex border border-gray-200 rounded overflow-hidden">
              {[['grid', <><rect x="3" y="3" width="7" height="7" rx="1"/><rect x="14" y="3" width="7" height="7" rx="1"/><rect x="3" y="14" width="7" height="7" rx="1"/><rect x="14" y="14" width="7" height="7" rx="1"/></>],
                ...
                <button aria-label="Thao tác" type="button" key={v} onClick={() => setView(v)}
            ...
            {/* Sort */}
            <select value={sort} onChange={e => setSort(e.target.value)} aria-label="Sắp xếp danh sách yêu thích"
                ...
                return (
                  <div key={item.id}
                    ...
                    <div className="p-3.5">
                      ...
                      <button aria-label="Thao tác" type="button" disabled={item.stock === 'out'}
