...
export default function AccountSection({ profile, onProfileUpdate }) {
  ...
  const strengthLabel = STRENGTH_LABEL
  const strengthColor = STRENGTH_COLOR
  ...
  return (
    <div className="space-y-5 text-gray-800">
      ...
      <div className="bg-white rounded border border-gray-200 shadow-sm overflow-hidden">
        ...
        <div className="px-8 pb-6 -mt-12 flex items-end gap-6">
          ...
          <div className="flex items-center gap-2 pt-14">
            ...
            {editing ? (
              <>
                <button aria-label="Đóng" type="button" onClick={handleCancel} className="px-4 py-2 text-sm font-semibold text-gray-600 border border-gray-300 rounded hover:bg-gray-50 transition-colors cursor-pointer">Huỷ</button>
                <button aria-label="Thao tác" type="button" onClick={handleSave} className="px-5 py-2 text-sm font-bold text-white bg-[var(--accent)] hover:bg-[var(--accent-d)] rounded transition-colors shadow-sm cursor-pointer border-none">Lưu thay đổi</button>
              </>
            ) : (
              <button aria-label="Thao tác" type="button" onClick={() => setEditing(true)} className="flex items-center gap-2 px-5 py-2 text-sm font-semibold text-gray-700 border border-gray-300 hover:border-[var(--accent)] hover:text-[var(--accent)] rounded transition-colors shadow-sm cursor-pointer bg-white">
                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      <div className="bg-white rounded border border-gray-200 shadow-sm">
        ...
        <div className="px-8 py-2">
          <Field label="Họ" value={info.firstName} editing={editing}>
            <input aria-label="Nhập họ" value={draft.firstName} onChange={handleChangeField('firstName')} className="w-full border border-gray-300 rounded px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8420A]/30 focus:border-[var(--accent)]" placeholder="Nhập họ" />
          </Field>
          <Field label="Tên" value={info.lastName} editing={editing}>
            <input aria-label="Nhập tên" value={draft.lastName} onChange={handleChangeField('lastName')} className="w-full border border-gray-300 rounded px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8420A]/30 focus:border-[var(--accent)]" placeholder="Nhập tên" />
          </Field>
          <Field label="Số điện thoại" value={info.phone.replace(/^(\d{3})\d{4}(\d{2})$/, '$1·····$2')} editing={editing} verified>
            <div className="flex gap-2">
              ...
              <input aria-label="Số điện thoại" value={draft.phone} onChange={handleChangeField('phone')} className="flex-1 border border-gray-300 rounded px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8420A]/30 focus:border-[var(--accent)]" placeholder="Số điện thoại" maxLength={10} />
            </div>
          </Field>
          <Field label="Email" value={info.email} editing={editing} verified>
            <div className="flex gap-2">
              <input aria-label="Địa chỉ email" value={draft.email} onChange={handleChangeField('email')} type="email" className="flex-1 border border-gray-300 rounded px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8420A]/30 focus:border-[var(--accent)]" placeholder="Địa chỉ email" />
              <button aria-label="Thao tác" type="button" className="text-sm font-semibold text-[var(--accent)] border border-orange-200 hover:bg-orange-50 px-4 rounded transition-colors shrink-0 cursor-pointer bg-white">Xác minh</button>
            </div>
          </Field>
          ...
          <Field label="Giới thiệu" value={info.bio || <span className="text-gray-400 italic">Chưa có thông tin</span>} editing={editing}>
            <textarea aria-label="Viết vài dòng giới thiệu về bản thân..." value={draft.bio} onChange={handleChangeField('bio')} rows={3} className="w-full border border-gray-300 rounded px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8420A]/30 focus:border-[var(--accent)] resize-none" placeholder="Viết vài dòng giới thiệu về bản thân..." />
          </Field>
        </div>
        ...
        {editing && (
          <div className="px-8 py-4 border-t border-gray-100 bg-gray-50/60 flex items-center justify-between rounded-b">
            ...
            <div className="flex gap-2">
              <button aria-label="Đóng" type="button" onClick={handleCancel} className="px-4 py-2 text-sm font-semibold text-gray-600 border border-gray-300 rounded hover:bg-gray-100 transition-colors cursor-pointer bg-white">Huỷ bỏ</button>
              <button aria-label="Thao tác" type="button" onClick={handleSave} className="px-6 py-2 text-sm font-bold text-white bg-[var(--accent)] hover:bg-[var(--accent-d)] rounded transition-colors shadow-sm cursor-pointer border-none">Lưu thay đổi</button>
            </div>
          </div>
        )}
      </div>
      ...
      {/* ── Password change ── */}
      <div className="bg-white rounded border border-gray-200 shadow-sm overflow-hidden">
        <button aria-label="Thao tác" type="button" onClick={() => setPwSection(v => !v)} className="w-full flex items-center justify-between px-8 py-5 hover:bg-gray-50/60 transition-colors bg-transparent border-none cursor-pointer text-left">
          ...
          <div className="px-8 pb-6 border-t border-gray-100">
            <div className="pt-5 space-y-4 max-w-md">
              {[
                ...
                <div key={f.key}>
                  <span className="block text-sm font-semibold text-gray-700 mb-1.5">{f.label}</span>
                  <div className="relative">
                    <input type={pwVisible[f.key] ? 'text' : 'password'} aria-label={f.label} value={pw[f.key]} onChange={setPwField(f.key)} placeholder={f.placeholder} className="w-full border border-gray-300 rounded px-4 py-2.5 pr-11 text-sm focus:outline-none focus:ring-2 focus:ring-[#E8420A]/30 focus:border-[var(--accent)]" />
                    <button aria-label={pwVisible[f.key] ? "Ẩn mật khẩu" : "Hiện mật khẩu"} type="button" onClick={() => togglePwVisible(f.key)} className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600 transition-colors border-none bg-transparent cursor-pointer">
                      {pwVisible[f.key] ? (
              ...
              <button aria-label="Thao tác" type="button" onClick={handleSavePw} className="px-6 py-2.5 text-sm font-bold text-white bg-[var(--accent)] hover:bg-[var(--accent-d)] rounded transition-colors shadow-sm border-none cursor-pointer">Xác nhận đổi mật khẩu</button>
            </div>
          </div>
        )}
      </div>
      ...
      <div className="bg-white rounded border border-gray-200 shadow-sm p-8">
        ...
        <div className="space-y-4">
          <div className="flex items-center justify-between p-4 rounded bg-gray-50 border border-gray-200">
            ...
            <button aria-label="Thao tác" type="button" onClick={() => setTwoFa(v => !v)} className={`relative w-12 h-6 rounded-full transition-colors duration-200 border-none cursor-pointer ${twoFa ? 'bg-[var(--accent)]' : 'bg-gray-300'}`}>
          ...
          <div className="p-4 rounded bg-gray-50 border border-gray-200">
            <div className="flex items-center justify-between mb-3">
              ...
              <button aria-label="Thao tác" type="button" className="text-xs font-bold text-red-500 hover:text-red-700 transition-colors border-none bg-transparent cursor-pointer">Đăng xuất tất cả</button>
            </div>
              ...
              {[
                ...
                <div key={d.name} className="flex items-center gap-3">
                  ...
                  {d.current
                    ...
                    : <button aria-label="Thao tác" type="button" className="text-xs font-semibold text-red-400 hover:text-red-600 transition-colors border-none bg-transparent cursor-pointer">Đăng xuất</button>
                  }
                </div>
              ))}
            </div>
