  ...
  return (
    <div className="flex flex-col gap-5">
      <div className="flex items-center gap-2.5">
        ...
        <span className="text-sm" style={{ color: 'var(--ct3)' }}>128 đánh giá</span>
      </div>

      <h1 className="text-2xl font-bold leading-tight" style={{ color: 'var(--ct1)', fontFamily: 'Syne, sans-serif' }}>Apple iPhone 15 Pro Max</h1>
      <p className="text-sm leading-relaxed" style={{ color: 'var(--ct2)' }}>Khung Titanium cao cấp · Chip A17 Pro · Nút tác vụ · Cổng USB-C</p>

      {/* Price */}
      <div className="flex items-center justify-between py-4" style={{ borderTop: '1px solid var(--cb)', borderBottom: '1px solid var(--cb)' }}>
        <div>
          <p className="text-[10px] mb-1 uppercase tracking-widest font-bold" style={{ color: 'var(--ct3)' }}>Giá bán</p>
          <p className="text-[34px] font-black leading-none" style={{ color: 'var(--accent)', fontFamily: 'Syne, sans-serif' }}>
      ...
      {/* Storage */}
      <div>
        <p className="text-[10px] font-bold uppercase tracking-widest mb-2.5" style={{ color: 'var(--ct3)' }}>Dung lượng lưu trữ</p>
        <div className="flex gap-2">
          {STORAGES.map(s => (
      ...
      {/* RAM */}
      <div>
        <p className="text-[10px] font-bold uppercase tracking-widest mb-2.5" style={{ color: 'var(--ct3)' }}>RAM</p>
        <div className="flex gap-2">
          {RAMS.map(r => (
      ...
      <div className="grid grid-cols-2 mt-1" style={{ borderTop: '1px solid var(--cb)' }}>
        {[
          ...
          <div key={title} className="flex items-center gap-3 px-3 py-3" style={{ borderRight: i === 0 ? '1px solid var(--cb)' : 'none' }}>
            ...
            <div>
              <p className="text-xs font-bold" style={{ color: 'var(--ct1)' }}>{title}</p>
              <p className="text-[11px] mt-0.5" style={{ color: 'var(--ct3)' }}>{sub}</p>
            </div>
          </div>
        ))}
      </div>
    ...
    <div className="flex gap-6 mt-6">
      <div className="flex-1 overflow-hidden" style={{ border: '1.5px solid var(--cb)', borderRadius: '16px', boxShadow: '0 4px 20px rgba(0,0,0,0.04)' }}>
        <div className="flex items-center gap-2 px-5 py-4" style={{ borderBottom: '1px solid var(--cb)', backgroundColor: 'var(--page)' }}>
          ...
          <h3 className="text-sm font-bold" style={{ color: 'var(--ct1)', fontFamily: 'Syne, sans-serif' }}>Thông số chi tiết</h3>
        </div>
...
function ProductTabs() {
  ...
  return (
    <div className="mt-10">
      ...
      {activeTab === 'desc' && <div className="mt-6 text-sm" style={{ color: 'var(--ct3)' }}>Mô tả sản phẩm đang được cập nhật...</div>}
      {activeTab === 'reviews' && <div className="mt-6 text-sm" style={{ color: 'var(--ct3)' }}>Đánh giá đang được tải...</div>}
    </div>
  )
}
  ...
  return (
    <div className="flex-1 flex flex-col min-h-screen" style={{ backgroundColor: 'var(--page)' }}>
      ...
      <div className="flex-1 w-full max-w-7xl mx-auto px-8 py-6">
        {/* Breadcrumb */}
        <nav className="flex items-center gap-1 text-sm mb-6" style={{ color: 'var(--ct3)' }}>
          <span onClick={() => onNavigate('home')} className="cursor-pointer transition-colors" style={{ color: 'var(--ct3)' }}
          ...
          <span onClick={() => onNavigate('list')} className="cursor-pointer transition-colors" style={{ color: 'var(--ct3)' }}
            onMouseEnter={e => e.currentTarget.style.color = 'var(--ct1)'}
            onMouseLeave={e => e.currentTarget.style.color = 'var(--ct3)'}
          >Điện thoại di động</span>
          <svg className="w-3 h-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 5l7 7-7 7" /></svg>
          <span style={{ color: 'var(--ct1)', fontWeight: 500 }}>Apple iPhone 15 Pro Max</span>
        </nav>
