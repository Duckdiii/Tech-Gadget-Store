...
const BG_CYCLE = ['bg-blue-500','bg-teal-500','bg-purple-500','bg-pink-500','bg-orange-500','bg-indigo-500','bg-green-500','bg-red-400','bg-cyan-500','bg-yellow-500']

const INIT_ACCOUNTS = [
  { id:1,  username:'admin.tran',    email:'van.an@techstore.vn',      name:'Trần Văn An',     role:'manager',  status:'active',  createdAt:'12/01/2023', lastLogin:'2 giờ trước',     loginCount:284, initials:'TA', bg:'bg-blue-500'   },
  ...
  { id:6,  username:'cust.tran2',    email:'tranthib@gmail.com',        name:'Trần Thị B',      role:'customer', status:'active',  createdAt:'10/02/2024', lastLogin:'1 tuần trước',     loginCount:14,  initials:'TB', bg:'bg-indigo-500' },
    ...
    <>
      <div className="fixed top-0 right-0 h-full w-[460px] bg-white shadow-2xl z-50 flex flex-col">
        ...
        <div className="bg-gradient-to-br from-slate-700 to-slate-900 px-6 pt-5 pb-6 relative">
          <button onClick={onClose} className="absolute top-4 right-4 p-1.5 rounded-lg hover:bg-white/10 cursor-pointer">
        ...
        <div className="px-6 py-4 border-t border-gray-100 space-y-2">
          <button onClick={() => setResetConfirm(true)} className="w-full flex items-center justify-center gap-2 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-700 hover:bg-gray-50 cursor-pointer transition-colors">
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 7a2 2 0 012 2m4 0a6 6 0 01-7.743 5.743L11 17H9v2H7v2H4a1 1 0 01-1-1v-2.586a1 1 0 01.293-.707l5.964-5.964A6 6 0 1121 9z" /></svg>
            Đặt lại mật khẩu
          </button>
          <div className="flex gap-2">
            {account.status === 'blocked'
              ? <button onClick={() => { onUnblock(account.id); onClose() }} className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-green-200 rounded-xl text-sm font-medium text-green-700 hover:bg-green-50 cursor-pointer transition-colors">
                  <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M8 11V7a4 4 0 118 0m-4 8v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2z" /></svg>
                  Mở khoá
                </button>
              : <button onClick={() => setBlockConfirm(true)} className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-orange-200 rounded-xl text-sm font-medium text-orange-600 hover:bg-orange-50 cursor-pointer transition-colors">
            ...
            <button onClick={() => setDeleteConfirm(true)} className="flex-1 flex items-center justify-center gap-2 py-2.5 border border-red-200 rounded-xl text-sm font-medium text-red-600 hover:bg-red-50 cursor-pointer transition-colors">
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[60]">
            <div className="bg-white rounded-2xl shadow-2xl w-[380px] p-6">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={() => setBlockConfirm(false)} className="flex-1 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 cursor-pointer">Huỷ</button>
                <button onClick={() => { onBlock(account.id); setBlockConfirm(false); onClose() }} className="flex-1 py-2.5 bg-orange-500 hover:bg-orange-600 text-white rounded-xl text-sm font-semibold cursor-pointer transition-colors">Khoá tài khoản</button>
              </div>
            </div>
          </div>
        </>
        ...
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[60]">
            <div className="bg-white rounded-2xl shadow-2xl w-[380px] p-6">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={() => setDeleteConfirm(false)} className="flex-1 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 cursor-pointer">Huỷ bỏ</button>
                <button onClick={() => { onDelete(account.id); setDeleteConfirm(false); onClose() }} className="flex-1 py-2.5 bg-red-600 hover:bg-red-700 text-white rounded-xl text-sm font-semibold cursor-pointer transition-colors">Xác nhận xoá</button>
              </div>
            </div>
          </div>
        </>
        ...
        <>
          ...
          <div className="fixed inset-0 flex items-center justify-center z-[60]">
            <div className="bg-white rounded-2xl shadow-2xl w-[380px] p-6">
              ...
              <div className="flex gap-3 mt-6">
                <button onClick={() => setResetConfirm(false)} className="flex-1 py-2.5 border border-gray-200 rounded-xl text-sm font-medium text-gray-600 hover:bg-gray-50 cursor-pointer">Huỷ</button>
                <button onClick={handleReset} className="flex-1 py-2.5 bg-blue-600 hover:bg-blue-700 text-white rounded-xl text-sm font-semibold cursor-pointer transition-colors">Gửi email</button>
              </div>
            </div>
          </div>
        </>
      ...
      <div className="flex-1 px-8 py-7 space-y-5">
        ...
        <div className="grid grid-cols-4 gap-4">
          {[
            ...
            const clr = { blue:['bg-blue-500','text-blue-600'], green:['bg-green-500','text-green-600'], red:['bg-red-500','text-red-600'], amber:['bg-amber-400','text-amber-600'] }[c.color]
            return (
              <div key={i} className="bg-white rounded-xl border border-gray-200 p-5 flex items-center gap-4">
                <span className={`w-12 h-12 ${clr[0]} rounded-xl flex items-center justify-center text-white shrink-0`}>{c.icon}</span>
        ...
        <div className="bg-white rounded-xl border border-gray-200 px-5 py-3.5 flex items-center gap-3">
          <div className="relative flex-1 max-w-sm">
            <svg className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M21 21l-6-6m2-5a7 7 0 11-14 0 7 7 0 0114 0z" /></svg>
            <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Tên, username, email..." className="w-full pl-9 pr-3 py-2 border border-gray-200 rounded-lg text-sm focus:outline-none focus:ring-2 focus:ring-blue-400" />
          </div>
                ...
                : filtered.map(acc => {
                  ...
                  return (
                    <tr key={acc.id} className="hover:bg-gray-50/70 transition-colors group">
                      ...
                      <td className="px-4 py-4">
                        <button onClick={() => setSelected(acc.id)} className="opacity-0 group-hover:opacity-100 transition-opacity text-xs text-blue-600 hover:text-blue-700 font-medium cursor-pointer px-2 py-1 rounded hover:bg-blue-50">
                          Chi tiết →
                        </button>
                      </td>
                    </tr>
